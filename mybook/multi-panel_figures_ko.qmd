...
작은 다중 그림 예제와 달리 복합 그림의 개별 패널에는 알파벳 레이블이 지정되어 있습니다. 라틴 알파벳의 소문자 또는 대문자를 사용하는 것이 일반적입니다. 특정 패널을 고유하게 지정하려면 레이블링이 필요합니다. 예를 들어 그림 @fig-BA-degrees-compound에서 학위 수여 비율의 변화를 보여주는 부분에 대해 이야기하고 싶다면 해당 그림의 (b) 패널 또는 간단히 그림 @fig-BA-degrees-compoundb를 참조할 수 있습니다. 레이블링이 없으면 그림 @fig-BA-degrees-compound의 "오른쪽 패널" 또는 "왼쪽 패널"에 대해 어색하게 이야기해야 하며 더 복잡한 패널 배열의 경우 특정 패널을 참조하는 것이 훨씬 더 어색할 것입니다. 작은 다중 그림의 경우 각 패널이 그림 레이블로 제공되는 패싯 변수에 의해 고유하게 지정되므로 레이블링이 필요하지 않으며 일반적으로 수행되지 않습니다.
