...
이 책 전체에서 저는 종종 동일한 그림의 다른 버전을 보여주는데, 일부는 좋은 시각화를 만드는 방법의 예이고 일부는 그렇지 않은 방법의 예입니다. 어떤 예를 모방해야 하고 어떤 예를 피해야 하는지에 대한 간단한 시각적 지침을 제공하기 위해 문제 있는 그림에 "보기 흉함", "나쁨" 또는 "잘못됨"으로 명확하게 레이블을 지정합니다(그림 @fig-ugly-bad-wrong-examples).
